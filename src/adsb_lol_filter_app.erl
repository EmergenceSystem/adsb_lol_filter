...
start(_Type, _Args) ->
    case adsb_lol_filter_sup:start_link() of
        {ok, Pid} ->
...
stop(_State) ->
    catch cowboy:stop_listener(adsb_lol_filter_query_listener),
    catch em_pop_sup:stop_node(adsb_lol_filter),
    ok.
...
start_pop_and_http() ->
    PopPort   = application:get_env(adsb_lol_filter, pop_port,   9400),
    QueryPort = application:get_env(adsb_lol_filter, query_port, 9401),
    Seeds     = application:get_env(adsb_lol_filter, pop_seeds,  []),
    Vec = em_filter_vec:from_capabilities(base_capabilities()),
    catch em_pop_sup:stop_node(adsb_lol_filter),
    catch cowboy:stop_listener(adsb_lol_filter_query_listener),
    {ok, PopPid} = em_pop_sup:start_node(adsb_lol_filter, #{
        port            => PopPort,
    ...
    Dispatch = cowboy_router:compile([
        {'_', [{"/agent/query", em_filter_http,
                #{server => adsb_lol_filter_server}}]}
    ]),
    {ok, _} = cowboy:start_clear(adsb_lol_filter_query_listener,
                                  [{port, QueryPort}],
                                  #{env => #{dispatch => Dispatch}}),
    logger:notice("[adsb_lol_filter] gossip port ~w  query port ~w",
                  [PopPort, QueryPort]),
    ok.
